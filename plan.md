# 대형마트 혼잡도 예측 서비스 기획서 (v1.3)

## 1. 개요

**1.1. 프로젝트 목표**
- 사용자가 지정한 날짜와 대형마트의 시간대별 예상 혼잡도를 제공하며, 추가적으로 선택된 날짜와 지역의 **예상 기온 정보**를 함께 표시하여, 사용자가 보다 쾌적하게 쇼핑 계획을 세울 수 있도록 돕는 웹 애플리케이션을 개발합니다.
...
**1.3. 타겟 사용자**
- 대형마트 방문 시 붐비는 시간을 피하고 싶거나, 날씨 정보도 함께 고려하고 싶은 모든 소비자
...
- **선택된 날짜 및 지역의 예상 기온 (예: 최고/최저 기온, 또는 평균 기온)**

## 3. 데이터 아키텍처

**3.1. 데이터 소스**
- **주 소스 (Primary): Cloudflare API (혼잡도)**
...
- **기온 API (Temperature API)**
  - 선택된 날짜와 지역에 대한 예상 기온 정보를 가져옵니다.
  - API 명세 (가정): `GET /weather?date=YYYY-MM-DD&region=REGION_CODE`
  - 예상 응답 (JSON): `{"min_temp": 5, "max_temp": 12, "avg_temp": 8}`
- **대체 소스 (Fallback): 통계 기반 추정 모델 (혼잡도)**
  - Cloudflare API 호출에 실패하거나, 응답이 없는 경우 또는 개발 초기 단계에서 아래의 자체 추정 모델을 사용하여 예상 혼잡도를 제공합니다.
...
- **JavaScript**:
  - `fetch` API를 사용하여 Cloudflare API (혼잡도) 및 기온 API에 비동기 요청을 보냅니다.
...
- **하단 (결과 영역):**
  ...
  - **선택된 날짜와 지역의 예상 기온 정보 (예: "예상 기온: 최저 5°C / 최고 12°C")**
...
4.  **JavaScript API 연동 모듈 개발 (혼잡도):**
    - `main.js`에 `fetch`를 사용하여 지정된 Cloudflare API를 호출하는 함수를 구현합니다.
    - 성공, 실패 등 다양한 응답에 대응하는 로직을 포함합니다.
5.  **JavaScript 기온 API 연동 모듈 개발:**
    - `main.js`에 `fetch`를 사용하여 지정된 기온 API를 호출하는 함수를 구현합니다.
    - 기온 정보를 가져오고 처리하는 로직을 포함합니다.
6.  **JavaScript Fallback 로직 개발 (혼잡도):**
    - 기존의 통계 기반 추정 모델 알고리즘을 별도 함수로 구현합니다.
7.  **DOM 연동 및 제어 로직 구현:**
    - "혼잡도 확인하기" 버튼 클릭 이벤트를 처리합니다.
    - 혼잡도 및 기온 API를 호출하고, 실패 시 혼잡도 Fallback 로직을 실행하여 최종 결과를 화면에 동적으로 표시합니다.
8.  **테스트 및 디버깅:** 기능이 의도대로 동작하는지 확인하고 오류를 수정합니다.
